...
sbgn.nodes = function(input.sbgn,
    ...
    for(i in seq_len(length.out = length(input.sbgns))){
        ...
        if(if.other.id.types.available ){
            all.id.mapping.result = load.all.ids.mapping(database,all.pairs.id.mapping.list,species
                                                         ,output.gene.id.type
                                                       ,output.cpd.id.type
                                                       ,SBGNview.data.folder = SBGNview.data.folder
                                                       )
...
get.arc.segments = function(
            ...
            for(i in seq_len(length(children))){
                  ...
                  if (xml2::xml_name(child) == "start" ){ # state variables has different shape in PD and ER, so we need to switch the shape when needed
                      ...
                      arc = new(
                          "next.sbgn.arc"
                          ,id = paste(arc.line["id"],arc.line["start.x"],sep="_")
                          ,start.x=as.numeric(arc.line["start.x"]),start.y=as.numeric(arc.line["start.y"]),end.x = as.numeric(arc.line["end.x"]),end.y=as.numeric(arc.line["end.y"]))
                      ...
                      arc = new(paste(arc.class,".sbgn.arc",sep="")
                                  ,id = paste(arc.line["id"],arc.line["start.x"],sep="_")
                                ,start.x=as.numeric(arc.line["start.x"]),start.y=as.numeric(arc.line["start.y"]),end.x = as.numeric(arc.line["end.x"]),end.y=as.numeric(arc.line["end.y"]))
...
break.text.into.segments = function(label,w,glyph.class,global.parameters.list,max.x=0,glyph){
    if.long.word = FALSE
