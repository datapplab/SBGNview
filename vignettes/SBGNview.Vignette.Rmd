...
# ***NOTE: loading 'sbgn.xmls' takes a few seconds since is a large data object  
#          containing the pre-generated SBGN-ML files.  
data("sbgn.xmls")
...
### Change output file name of a *SBGNview* object
...
More graph features can be controlled by directly modifying the *SBGNview* object.
...
For each input sbgn (pathway IDs from `data("pathways.info")` or local SBGN-ML files) in *SBGNview* object, the 'global.parameters.list' element contains a record of graphical parameters passed from *SBGNview* function to *renderSbgn* function. Run `?"renderSbgn"` for more information. 
...
# ID mapping
SBGNview can automatically map common ID types to SBGN-ML glyphs in our [pre-generated SBGN-ML files](https://github.com/datapplab/SBGNhub/tree/master/data/SBGN.with.stamp).  Supported ID types can be accessed as follow:
```{r , echo = TRUE, message = FALSE, warning = FALSE}
data("mapped.ids")
```
